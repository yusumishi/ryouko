...
#        print(user_links)
    ...
    def buildToolBar(self):
        if self.page().mainFrame().findFirstElement("#ryouko-toolbar").isNull() == True:
            self.page().mainFrame().findFirstElement("body").appendInside("""<style type="text/css">#footer_homepage, #fbDockChat, [class="Hoa n0b lla MQ"]{bottom:25px !important;}html{padding-bottom: 24px;}#ryouko-toolbar {overflow-y: auto; height: 24px; width: 100%; left: 0;padding: 0;background: ThreeDFace;position: fixed;visibility: visible;z-index: 9001;}#ryouko-toolbar *{font-family: sans-serif; text-align: left; margin: 0; font-size: 14px; background: transparent; padding: 0; border: 0; color: ButtonText; text-decoration: none; -webkit-appearance: none;} #ryouko-toolbar a:hover, #ryouko-toolbar input:hover{text-decoration: underline; }</style><span id='ryouko-toolbar' style='bottom: 0; border-top: 1px solid ThreeDShadow;'><span id='ryouko-browser-controls'></span><span id='ryouko-link-bar-container'></span><a id='ryouko-switch-button' style='float: right; padding-left: 4px; padding-right: 4px; margin: 0;' href="javascript:if (document.getElementById('ryouko-toolbar').getAttribute('style')=='top: 0; border-bottom: 1px solid ThreeDShadow;') { document.getElementById('ryouko-toolbar').setAttribute('style', 'bottom: 0; border-top: 1px solid ThreeDShadow;'); document.getElementsByTagName('html')[0].setAttribute('style', 'padding-top: 0; padding-bottom: 24px;'); document.getElementById('ryouko-switch-button').innerHTML = '^'; } else { document.getElementById('ryouko-toolbar').setAttribute('style', 'top: 0; border-bottom: 1px solid ThreeDShadow;'); document.getElementsByTagName('html')[0].setAttribute('style', 'padding-top: 24px; padding-bottom: 0;'); document.getElementById('ryouko-switch-button').innerHTML = 'v'; }">^</a></span>""")
